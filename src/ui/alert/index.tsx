...
export interface AlertOptions {
	...
	showCancel?: boolean;
	/**
	 * Destructive 액션 여부. true 시 확인 버튼이 outline(secondary)으로 변환되고
	 * 취소 버튼이 강조됨 — 실수 클릭 방지 (Apple HIG 패턴).
	 * @default false
	 */
	destructive?: boolean;
	/** 액션 버튼 정렬 (기본값: "right") */
...
const AlertModal: React.FC<AlertModalProps> = ({
	...
	// destructive: confirm을 outline(secondary)으로, cancel을 filled(primary)로 — 실수 방지
	const confirmVariant: ButtonVariant = destructive ? "outline" : "filled";
	const cancelVariant: ButtonVariant = destructive ? "filled" : "outline";
		...
		>
			...
			>
				...
				<div className={cn("alert_actions", `alert_actions_${actionsAlign}`)}>
					...
					<Button variant={confirmVariant} size="md" radius="md" onClick={onConfirm}>
						{confirmText}
					</Button>
				</div>
			</div>
