    ...
    return (
        ...
        >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                />
                <div>
                    <code style={{ fontSize: 12 }}>{group}.{token}</code>
...
function Section({ title, description, group, entries }: {
    title: string;
    description: string;
    group: string;
...
export const Semantic: Story = {
    ...
    render: () => (
        <div style={{ display: "grid", gap: 32, maxWidth: 760 }}>
            <Section title="Brand" description="브랜드 주요 색상" group="colors.brand"
                entries={Object.entries(colors.brand as ColorGroup)} />
            <Section title="Text" description="텍스트 계층" group="colors.text"
                entries={Object.entries(colors.text as ColorGroup)} />
            <Section title="Background" description="배경색" group="colors.bg"
                entries={Object.entries(colors.bg as ColorGroup)} />
            <Section title="State" description="인터랙션 상태 (hover, pressed)" group="colors.state"
                entries={Object.entries(colors.state as ColorGroup)} />
            <Section title="Border" description="구분선" group="colors.border"
                entries={Object.entries(colors.border as ColorGroup)} />
            <Section title="Status" description="성공 / 오류 / 경고 / 정보" group="colors.status"
