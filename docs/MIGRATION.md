...
> 이 변경은 3.21.1 에 먼저 실려 나갔습니다. 소비자 코드가 깨지는 변경이라 patch 가 아니라 minor 가 맞아, 같은 내용을 3.22.0 으로 다시 올리고 이 문서를 붙였습니다. **3.21.1 을 이미 받았다면 동작은 3.22.0 과 같습니다.**
...
## v3.21.0 (오버레이 잠금이 거터를 회수한다)

타입·prop 변경은 없습니다. 잠금 동안의 레이아웃 처리가 바뀌어, **오른쪽에 고정된 앱 요소**가 영향을 받습니다.

### 무엇이 바뀌나

Modal · Drawer · Alert 가 열려 있는 동안 DS 가 스크롤바 자리(또는 앱이 `scrollbar-gutter: stable` 로 예약해 둔 거터)를 **회수하고** 그 폭만큼 `body` 에 `padding-right` 를 줍니다. 3.17~3.20 은 반대로 거터를 예약해 두고 그 띠를 루트 배경색으로 칠했는데, 배경이 단색이 아니면 경계에 세로선이 남았습니다(#635).

### 앱 쪽에서 할 일

- **오른쪽에 고정된 요소**(FAB · 플로팅 툴바 · 자체 토스트)가 있으면 회수된 폭을 더해 제자리를 지키세요. 안 하면 오버레이가 열린 동안 스크롤바 폭만큼 오른쪽으로 움직입니다.

  ```css
  .my-fab { right: calc(16px + var(--bt-scrollbar-width, 0px)); }
  ```

- 3.17~3.20 에서 "보정을 걷어내라" 는 안내를 따라 지웠던 앱은 이 보정을 **다시 넣어야** 합니다.
- 자체 오버레이를 가운데 정렬해 두었다면 컨테이너에 `padding-right: var(--bt-scrollbar-width, 0px)` 를 주세요 - DS 오버레이가 쓰는 것과 같은 상쇄입니다.
- 루트 배경색을 잠금 중에 읽어 무언가를 하던 코드가 있다면 더 이상 바뀌지 않습니다.

`--bt-scrollbar-width` 의 의미(잠금 중 실측 폭, 밖에서는 `0px`)와 `html[data-bt-scroll-locked]` 표식은 그대로입니다.
