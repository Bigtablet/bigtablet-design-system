...
사라지면 콘텐츠 영역만 넓어지는 것이 아니라 **ICB(초기 컨테이닝 블록) 폭이 함께 변한다.** 그러면
`position: fixed; left: 50%` 처럼 폭에 비례하는 값을 쓰는 요소가 스크롤바 폭의 **절반**만큼 움직인다.

그래서 DS 는 잠금 동안 거터를 **예약한다**(`html { scrollbar-gutter: stable }`). 스크롤바가 사라져도
그 자리가 남아 ICB 폭이 변하지 않으므로, 흐름 안 콘텐츠도 고정 요소도 움직이지 않는다.

| 상황 | 잠금이 하는 일 |
| --- | --- |
| 문서가 스크롤되고 스크롤바가 있다 | 거터를 `stable` 로 예약. `padding` 보정 없음 |
| 앱이 이미 `scrollbar-gutter: stable` 이다 | 그대로 둔다 (폭이 변하지 않으므로) |
| 문서가 스크롤되지 않는다 (셸이 내부 컨테이너를 스크롤) | 폭 보정을 하지 않는다. 예약된 거터가 있으면 그 띠만 어둡게 칠한다 |
| `scrollbar-gutter` 미지원 (Safari 18.2 미만) | 기존 `body` `padding-right` 보정으로 폴백 |

잰 폭은 `:root` 의 `--bt-scrollbar-width` 로 노출된다(소비자가 거터 폭을 알아야 할 때를 위해).
잠금 밖에서는 `0px` 이라 `var()` 가 항상 유효하다. **앱 쪽 고정 요소를 이 변수로 보정할 필요가 없다** - ICB 폭이 변하지 않으므로
요소가 움직이지 않는다. 기존에 보정을 넣어 둔 앱은 그 보정을 걷어내면 된다(안 걷어내면 이제 그
보정만큼 어긋난다).

### 예약된 거터를 어둡게 하는 방법 (딤은 거기 못 닿는다)

예약된 거터는 **캔버스**(루트 요소의 배경)가 칠하는 영역이라 `html` 의 자손은 거기에 페인트할 수
...
elementFromPoint(1260, 250)            .modal
elementFromPoint(1266, 250)            null   ← 여기부터 아무것도 없다
```

루트의 `background-image` 도 거터로 전파되지 않는다(같은 실측). **전파되는 것은
...
바닥색은 캔버스가 실제로 쓰는 색을 그 순서로 찾는다 - 루트 배경색 → (투명하면) `body` 배경색 →
(둘 다 투명하면) 흰색. 딤 색은 `--bt-color-bg-overlay`(Vanilla `--bt-color-background-overlay`)
토큰에서 읽으므로 테마를 따른다. `scrollbar-gutter` 미지원 브라우저의 `padding` 폴백 경로에서는
거터 자체가 없으므로 칠하지 않는다.
...
html[data-bt-scroll-locked] .my-floating-toolbar { display: none; }
```

### 패널은 스크롤바폭/2 만큼 어긋난다 (의도)

dim 은 ICB 를 채우고, 패널은 그 안에서 중앙 정렬된다. 스크롤바(또는 거터)가 오른쪽 15px 을
차지하므로 패널 중심은 물리 화면 중앙보다 그만큼의 절반(7.5px) 왼쪽이다. **이건 고정 요소
이동(#574)과 다른 이야기이고 의도된 동작이다** - 패널은 방금 나타난 요소라 사용자에게 "움직였다"
고 인지될 기준점이 없고, 잠금 중에도 그 위치가 유지된다(ICB 가 변하지 않으므로).
