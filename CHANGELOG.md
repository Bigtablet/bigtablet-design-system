...
- Vanilla `Modal` 을 열린 채 `destroy()` 하면 `close()` 와 같은 정리를 합니다 - Escape 등록·스크롤 잠금에 더해 **포커스 복원과 우리가 붙인 `tabindex` 제거**까지. 지금까지는 포커스가 파괴된 패널 안에 남고, 같은 DOM 으로 다시 만든 `Modal` 이 남은 `tabindex` 를 "원래 있던 것" 으로 오인했습니다. `destroy()` 는 바인딩 해제라 `onClose` 는 부르지 않습니다
