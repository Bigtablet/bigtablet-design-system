...
- **Coverage**: 92.02% stmts / 89.1% branch / 92.77% funcs / 94.04% lines - **`unit` 프로젝트 기준**(스토리북 러너로 재면 훨씬 낮게 나오는데 정상이다). 자세한 표는 [docs/TESTING.md](./docs/TESTING.md#커버리지)
- **Commands**:
  ```bash
...
>
> **태그를 푼 뒤의 보고에는 검증 결과와 이번 릴리즈로 닫힌 이슈만 쓴다.** 남은 후속 항목·열린
