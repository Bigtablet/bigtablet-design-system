...
export const Pagination = ({ page, total, onChange, siblingCount = 1 }: PaginationProps) => {
    ...
    const pages: (number | "…")[] = [1, ...(start > 2 ? ["…"] as const : []), ...mid, ...(end < total - 1 ? ["…"] as const : []), total]
        .filter((x, i, arr) => (typeof x === "number" ? arr.indexOf(x) === i : true));

    return (
        <nav className="pagination" aria-label="Pagination">
            <button className="pagination__item" onClick={() => onChange(clamp(page - 1))} disabled={page <= 1}>Prev</button>
            {pages.map((p, i) =>
                typeof p === "number" ? (
                    <button key={i} className={["pagination__item", p === page && "is-active"].filter(Boolean).join(" ")} onClick={() => onChange(p)}>
                        {p}
                    </button>
                ) : (
            ...
            <button className="pagination__item" onClick={() => onChange(clamp(page + 1))} disabled={page >= total}>Next</button>
        </nav>
    );
};