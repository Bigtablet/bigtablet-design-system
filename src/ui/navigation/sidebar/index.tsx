...
export const Sidebar = ({
		...
		const autoOpen = items
			.filter(
				(item): item is SidebarGroupItem =>
					item.type === "group" && item.children.some((c) => isActive(c.href)),
			)
			.map((g) => g.id);
	...
	return (
		<aside className={sidebarClassName} style={style}>
			{isOpen ? (
				<>
					...
					<nav className="sidebar_nav" aria-label="메인 메뉴">
						{items.map((item) => {
							if (item.type === "group") {
								...
								const subClassName = ["sidebar_sub", open && "sidebar_sub_open"]
									.filter(Boolean)
									.join(" ");

								const chevronClassName = ["sidebar_chevron", open && "sidebar_chevron_open"]
									.filter(Boolean)
									.join(" ");
											...
											{item.children.map((child) => {
												...
												return (
													...
													>
														...
														<span className="sidebar_sub_label">{child.label}</span>
													</Link>
												);
											})}
										</div>
							...
							const itemClassName = ["sidebar_item", active && "sidebar_item_active"]
								.filter(Boolean)
								.join(" ");
				...
				<button type="button" className="sidebar_open_btn" onClick={() => toggleSidebar(true)}>
					<Image src="/images/sidebar/menu.svg" alt="사이드바 열기" width={24} height={24} />
				</button>
			)}
		</aside>
	);
