...
- **Dropdown**: 값을 선택 (controlled `value`/`onChange`). 폼 필드. `role="combobox"` 또는 `listbox`.
...
<Accordion items={faq} openKeys={open} onChange={setOpen} />
```
...
| `openKeys` | `string[]` | - | 제어형 펼침 키 (`onChange`와 함께 사용) |
...
<Accordion items={items} openKeys={open} onChange={(keys) => { setOpen(keys); syncToUrl(keys); }} />
```
