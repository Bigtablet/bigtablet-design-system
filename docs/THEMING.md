...
| 문서가 스크롤되지 않는다 (셸이 내부 컨테이너를 스크롤) | 폭 보정을 하지 않는다. 예약된 거터가 있으면 폭만 노출해 오버레이가 덮을 수 있게 한다 |
| `scrollbar-gutter` 미지원 (Safari 18.2 미만) | 기존 `body` `padding-right` 보정으로 폴백 |

잰 폭은 `:root` 의 `--bt-scrollbar-width` 로 노출된다. 잠금 밖에서는 `0px` 이라 `var()` 가 항상
유효하다. **앱 쪽 고정 요소를 이 변수로 보정할 필요가 없다** - ICB 폭이 변하지 않으므로
요소가 움직이지 않는다. 기존에 보정을 넣어 둔 앱은 그 보정을 걷어내면 된다(안 걷어내면 이제 그
보정만큼 어긋난다).

### 오버레이가 예약된 거터를 덮는 방법

예약된 거터는 `position: fixed` 의 컨테이닝 블록 **밖**이다. `inset: 0` 이나 `100vw` 로는 닿지 않는다
(Chromium 실측 - 뷰포트 1200·거터 15px 에서 둘 다 1185 로 잡힌다). 음수 오프셋만 닿는다. DS 의 dim 은
그래서 이렇게 넘어간다.
...
.modal {
  position: fixed;
  inset: 0;
  right: calc(-1 * var(--bt-scrollbar-width, 0px)); // 예약된 거터까지 덮는다
}
```

소비자가 전체 뷰포트를 덮는 자기 오버레이를 만들 때도 같은 한 줄이 필요하다.

### 패널은 여전히 스크롤바폭/2 만큼 어긋난다 (의도)

dim 이 예약된 거터까지 덮어 **물리 뷰포트 전폭**이 되고, 패널은 그 안에서 중앙 정렬된다. 그래서
패널 중심은 스크롤바가 없는 화면의 중앙에 잡히고, 스크롤바가 있던 상태의 시각적 중앙과는
`스크롤바폭 / 2`(위 예에서 7.5px) 어긋난다. **이건 고정 요소 이동(#574)과 다른 이야기이고 의도된
동작이다** - 패널은 방금 나타난 요소라 사용자에게 "움직였다" 고 인지될 기준점이 없고, 잠금 중에는
그 위치가 시각적 중앙이다.
