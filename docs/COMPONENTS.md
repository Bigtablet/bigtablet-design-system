...
> **v3.15 변경**: `h1`·`h2` 가 이전까지 본문과 같은 `400` 이었고 `h3`~`h6` 만 `700` 이라 굵기 계층이 뒤집혀 있었습니다. `h1` 이 `h3` 보다 가늘게 보였습니다. `bold` 변형으로 교체해 `h1` 은 자간도 `-0.01em` 이 됩니다(`lg` 에서는 `h2` 까지).
...
```scss
.policy :global(.prose) :global(h1) { padding-bottom: 0; border-bottom: 0; }
```
